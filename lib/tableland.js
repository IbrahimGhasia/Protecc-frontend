...
class TableLand {
  ...
  async checkExistingTable(name) {
    await this.checkConnected();
    const tables = await this.tableland.list(); // returns an Object with the Tables the connected address owns
    const myMatchingTables = Object.values(tables).filter((table) => table.name.includes(name)); // filters `tables` for matching `structure`s
    return myMatchingTables;
  }

  async createTable(prefix) {
    await this.checkConnected();
    const encryptionSchema = `name text, encryptedValue text, encryptedSymmetricKey text, primary key (name)`;
    // Create a table
    const { name } = await this.tableland.create(encryptionSchema, {
      prefix: prefix // Optional `prefix` used to define a human-readable string
    })
  ...
  async createAppointmentTable() {
    ...
    const { name } = await this.tableland.create(`name text, value text, primary key (name)`, {
      prefix: "appointmentStage2" // Optional `prefix` used to define a human-readable string
    })
    console.log("created table", name);
    return name;
  }
  ...
  async readAppointmentsFromTable(tableID) {
    await this.checkConnected();
    const readRes = await this.tableland.read(`SELECT * FROM ${tableID}`);
    let appointment = {};
    for (let idx in readRes.rows) {
      const record = readRes.rows[idx];
      appointment[record[0]] = record[1];
    }
    return appointment;
  }
